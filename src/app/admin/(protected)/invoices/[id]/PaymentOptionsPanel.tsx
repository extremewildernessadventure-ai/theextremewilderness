...
export default function PaymentOptionsPanel({
    ...
    return (
      <div className="panel" style={{ background: 'var(--green-bg)', borderColor: 'transparent' }}>
        <div className="flex items-start gap-3">
          <CheckCircle2 className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: 'var(--pine-fg)' }} />
          <div>
            <h2 className="text-[10px] font-black uppercase tracking-widest mb-1" style={{ color: 'var(--pine-fg)', fontFamily: 'var(--font-inter)' }}>Paid in Full</h2>
            {latestPayment ? (
              <p className="text-sm" style={{ color: 'var(--pine-fg)' }}>
              ...
              <p className="text-sm" style={{ color: 'var(--pine-fg)' }}>This invoice has been paid in full.</p>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    // --gold-bg (a translucent dark-friendly tint, matching the gold
    // "Payment Options" heading below), not --sand — --sand is now a
    // *text* color (cream, for use on dark backgrounds), not a surface;
    // it would render as a literal light-cream box here.
    <div className="panel space-y-4" style={{ background: 'var(--gold-bg)', borderColor: 'transparent' }}>
