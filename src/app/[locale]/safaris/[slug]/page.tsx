...
import { Clock, Users, Check, X, ChevronDown, Calendar, ShieldCheck, Star, MapPin, DollarSign, Sparkles } from 'lucide-react'
...
export default async function SafariPackagePage({ params }: Props) {
  ...
  // same lookup the filter sidebar uses.
  ...
  // Tier Level badge — derived from whichever pricing grid the package
  // actually has populated (Wilderness Trail/Reserve/Sovereign, or the
  // family Luxury/Ultra-Luxury pair).
  ...
  // months), which would overflow the sidebar's quick-metadata tile as a raw
  // comma join; those show a plain "Year-Round" label instead.
  ...
  return (
    <>
      ...
      {/* Badges + Share/Enquire actions + rating/operator/parks row — grafted
          in from the live site's title/metadata block, kept in this
          position (after the hero, before Overview) rather than replacing
          the hero the way the live site's own layout does. */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <ShareLinkButton title={t('shareLabel')} copiedLabel={t('shareCopied')} tooltip={t('shareTooltip')} />
            <BookNowButton
              packageName={pkg.name}
              packageType={TRIP_TYPE_LABEL[pkg.type]}
              priceFrom={`$${pkg.priceFrom.toLocaleString('en-US')}`}
              duration={`${pkg.duration} ${tc('days')}`}
              label={t('enquireButtonLabel')}
              arrow={false}
              className="inline-flex items-center justify-center whitespace-nowrap px-4 py-2 rounded-md bg-gold text-brand text-xs font-semibold uppercase tracking-wider hover:bg-gold-dark transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs mb-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {reviewStats.count > 0 && (
              <span className="text-text-muted">{t('packageReviewsCount', { count: reviewStats.count })}</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 text-text-muted flex-wrap">
            ...
            <span className="font-semibold text-brand">{pkg.operatorName ?? 'EWA Safari Outfitters'}</span>
            <OperatorBadge
              operatorName={pkg.operatorName ?? 'EWA Safari Outfitters'}
              directLabel={t('operatorDirectBadge')}
              partnerLabel={t('operatorPartnerBadge')}
            />
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <Reveal className="lg:col-span-2 space-y-10">
            ...
            {/* Wildlife Radar — real sighting-likelihood data for this
                itinerary's actual route/parks. Only rendered when the
                package actually has real data behind it. */}
            {pkg.wildlifeTargets && pkg.wildlifeTargets.length > 0 && (
              <SafariWildlifeTab
            ...
            {/* Photo Gallery — only rendered when the package actually has extra
                gallery photos beyond the hero. Kept in this position (below
                the Overview card, above At a Glance) even though the
                component's own prototype design has no separate hero banner
                above it elsewhere on the site. */}
            {pkg.gallery.length > 0 && (
              <div>
                ...
                <SafariPhotoGallery
                  ...
                  locationLabel={pkg.destinations.map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(', ')}
                />
              </div>
            )}
            ...
            {/* Guest Reviews & Ratings — real, staff-moderated reviews tied
                to this specific package (reviews.package_slug), not the
                single hardcoded testimonial above. Only the review section
                itself is brought back here, not the full prototype tabs
                layout it originally lived in. */}
            <div className="border border-brand/20 rounded-xl p-5">
            <SafariReviewsTab
