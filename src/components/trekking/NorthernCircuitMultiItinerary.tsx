...
// Unlike LemoshoMultiItinerary/MachameMultiItinerary (where `faq` is a single
// static prop shown regardless of the active tab), Northern Circuit's source
// content gives each duration its own dedicated FAQ that switches alongside
// the day-by-day content — so `faq` lives per-variant here, not as a
// top-level prop.
interface Variant {
...
interface Props {
  variants: { '8': Variant; '9': Variant; '10': Variant }
  included: string[]
  excluded: string[]
...
const TAB_KEYS: TabKey[] = ['8', '9', '10']

export default function NorthernCircuitMultiItinerary({
  variants,
  included,
  excluded,
  choosing,
  ...
  const [active, setActive] = useState<TabKey>('9')
  ...
  return (
    <div className="space-y-12">
      ...
      <div>
        <div className="flex gap-2 flex-wrap mb-8">
          {TAB_KEYS.map((key) => (
            <button
              key={key}
              onClick={() => { setActive(key); setOpenFaq(null) }}
      ...
      {/* ── FAQ — switches per active duration tab ────────────── */}
      <div>
